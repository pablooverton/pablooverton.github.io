...
Withdrawal sequencing drew from brokerage first (no MAGI impact), then fell through to uncapped pretax once the brokerage ran dry. Every subsidy gone.
...
I write data pipelines professionally. This failure mode lives there too. An ETL transform can be type-safe, unit-tested at the function level, and still produce analytically wrong output: a join type wrong, a null handling assumption off, a date truncation that didn't match the business definition. You only catch it if you have a verified reference case and fail explicitly when output diverges.
